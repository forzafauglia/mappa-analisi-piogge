...
if isinstance(df.columns, pd.MultiIndex): df.columns = [''.join(map(str, col)).strip() for col in df.columns.values]
cleaned_cols = {}
for col in df.columns:
cleaned_name = re.sub(r'[.?]|(.?)|'', '', str(col)).strip().replace(' ', '').upper()
...
if "Stamen" in tile:
return folium.Map(location=location, zoom_start=zoom, tiles=tile, attr='© <a href="https://www.stadiamaps.com/" target="_blank">Stadia Maps</a> © <a href="https://openmaptiles.org/" target="_blank">OpenMapTiles</a> © <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors')
